...
class test_layout(CarthageLayout, PublicDnsManagement, AnsibleModelMixin):
    ...
    add_provider(
        ...
        image_provider(owner=debian_ami_owner, name="debian-12-amd64-*"),
    )
    ...
    class test_vm(MachineModel):
        ...
        class static_ip_net_config(NetworkConfigModel):
            add(
                ...
                v4_config=V4Config(dhcp=False, address="192.168.100.237"),
            )
    ...
    class image_builder(
        ...
        config_info = mako_task(
            "config.yml.mako",
            output="carthage_aws/config.yml",
            config=InjectionKey(ConfigLayout),
        )
        ...
        class install(MachineCustomization):
            ...
            async def install_software(self):
                await self.ssh(
                    "apt -y install python3-pip rsync python3-pytest python3-netifaces ansible",
                    _bg=True,
                    _bg_exc=False,
                )
