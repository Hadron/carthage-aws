...
class AwsVolume(AwsManaged, InjectableModel):
    ...
    async def pre_create_hook(self):
        ...
        if not self.volume_size:
            self.volume_size = self._gfi("volume_size", default=None if self.snapshot else "error")
    ...
    async def attach(self, instance, device, delete_on_termination=True):
        ...
        def callback():
            self.connection.client.attach_volume(VolumeId=self.id, InstanceId=instance_id, Device=device)
            if delete_on_termination:
                self.connection.client.modify_instance_attribute(
                    InstanceId=instance_id,
                    BlockDeviceMappings=[{"DeviceName": device, "Ebs": {"DeleteOnTermination": delete_on_termination}}],
                )
            self.mob.reload()
    ...
    async def detach(self, instance, device):
        ...
        def callback():
            self.connection.client.detach_volume(VolumeId=self.id, InstanceId=instance_id, Device=device)
            self.mob.reload()
...
def attach_volume_task(*, device, volume, delete_on_termination=True):
    ...
    async def attach_volume(vm):
        ...
        if not vm.mob:
            return False
        return any(filter(lambda mapping: mapping["DeviceName"] == device, vm.mob.block_device_mappings))
