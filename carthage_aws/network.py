...
class AwsVirtualPrivateCloud(AwsManaged, ModelContainer):
    ...
    async def delete(self):
        ...
        for g in self.mob.security_groups.all():
            try:
                await run_in_executor(g.delete)
            except Exception:
                pass
...
class AwsSecurityGroup(AwsManaged, InjectableModel):
    ...
    async def read_write_hook(self):
        ...
        if not self.readonly:
            await run_in_executor(callback)
            try:
                del self.existing_ingress
            except Exception:
                pass
            try:
                del self.existing_egress
            except Exception:
                pass
    ...
    async def delete(self):
        if self.mob:
            try:
                ...
                if self.mob.association:
                    await run_in_executor(self.mob.association.delete)
            except Exception:
                pass
...
async def aws_link_handle_eip(model, link):
    if link.merged_v4_config.public_address:
        try:
            ...
            logger.warning(
                "%s interface %s has public address that cannot be assigned",
                model,
                link.interface,
            )
