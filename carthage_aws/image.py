...
def image_provider(name, *, owner="self", architecture="x86_64", all_images=False, fallback=None):
...
class AwsImage(AwsManaged):
    ...
    async def possible_ids_for_name(self):
        objs = self.connection.client.describe_images(
            Owners=["self"], Filters=[{"Name": "name", "Values": [self.name]}]
        )
...
@inject(model=AbstractMachineModel, volume_size=InjectionKey("aws_image_size"))
class ImageBuilderVolume(AwsVolume):
    async def pre_create_hook(self):
        ...
        self.injector.add_provider(InjectionKey("aws_availability_zone"), machine.mob.placement["AvailabilityZone"])
...
class AttachImageBuilderVolume(MachineCustomization, InjectableModel):
    ...
    def attach_volume(self):
        return any(filter(lambda mapping: mapping["DeviceName"] == "/dev/xvdi", self.host.mob.block_device_mappings))
...
class ImageBuilderPipeline(MachineCustomization, InjectableModel):
    ...
    def __init__(self, name, description=None, add_time_to_name=False, **kwargs):
        if add_time_to_name:
    ...
    @inject(
        container_image=InjectionKey(DebianContainerImage, _ready=False),
...
async def build_ami(name, *, injector, local_model=None, **kwargs):
    ainjector = injector(AsyncInjector)
    ...
    await machine.apply_customization(ImageBuilderPipeline, model=local_model, name=name, **kwargs)
