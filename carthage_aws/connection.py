    ...
    def invalid_ec2_resource(self, resource_type, resource_id, *, name=None):
        ...
        if name:
            ...
            if names and name in names:
                names[name] = list(filter(lambda r: r["ResourceId"] != resource_id, names[name]))
...
class AwsManaged(SetupTaskMixin, AsyncInjectable):
    ...
    def resource_tags(self):
        ...
        return {"ResourceType": self.resource_type.replace("_", "-"), "Tags": tags}
...
class AwsDeployableFinder(DeployableFinder):
    ...
    async def find(self, ainjector):
        ...
        results = await ainjector.filter_instantiate_async(
            None,
            lambda k: isinstance(k.target, type)
            and issubclass(k.target, AwsManaged)
            and not issubclass(k.target, AwsSubnet),
            ready=False,
            stop_at=ainjector,
        )
