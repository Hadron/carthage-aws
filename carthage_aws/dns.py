...
class AwsHostedZone(AwsManaged, DnsZone):
    ...
    def find_from_name(self):
        try:
            ...
            if len(r["HostedZones"]) > 0 and r["HostedZones"][0]["Name"] == self.name + ".":
    ...
    def do_create(self):
        try:
            r = self.client.create_hosted_zone(
                ...
                HostedZoneConfig={"Comment": "Created by Carthage", "PrivateZone": self.private},
            )
            self.mob = r
            # [12:] is because we want to trim `/hostedzone/` off of the zone Id
            ...
            logger.error("Could not create AwsHostedZone for %s because %s.", self.name, e)
        ...
        def callback():
            try:
                ...
                if changes:
                    ...
                    self.client.change_resource_record_sets(HostedZoneId=self.id, ChangeBatch=change_batch)
    ...
    async def update_records(self, *args, ttl=300):
        ...
        for a in args:
            ...
            changes.append(
                {
                    "Action": "UPSERT",
                    "ResourceRecordSet": {"Name": name, "Type": rrtype, "TTL": ttl, "ResourceRecords": records},
                }
            )
        try:
...
class AwsPrivateHostedZone(AwsHostedZone):
    ...
    def do_create(self):
        ...
        try:
            r = self.client.create_hosted_zone(
                Name=self.name,
                VPC={"VPCRegion": self.region, "VPCId": self.vpc_id},
                CallerReference=str(datetime.datetime.now().timestamp()),
                HostedZoneConfig={"Comment": "Created by Carthage", "PrivateZone": self.private},
            )
            self.mob = r
